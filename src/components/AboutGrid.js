...
const AboutGrid = ({icon,text,label,heading}) => {
  ...
  <div className='flex justify-center items-center gap-5 flex-col bg-white rounded-lg m-2 p-3 lg:p-7'>
    ...
    <p className='text-18 text-center text-slate-700'>{text}</p>
    <p className='bg-green-800 text-white px-6 py-2 rounded-md text-16 hover:bg-black hover:text-white cursor-pointer'>{label}</p>
  </div>
  )
}
